...
- precisa de uma pequena função para uma finalidade específica e temporária.
...
Pode retornar uma função lambda de outra função para criar um comportamento dinâmico.
...
- O lambda retornado é armazenado em `add5`, que adiciona `5` à a entrada.
...
- **Flexibilidade:** pode defini-los e usá-los onde for necessário, tornando o código mais modular.
...
- está trabalhando com funções de ordem superior como `map`, `filter` ou `reduce`.
...
As funções Lambda no Scheme fornecem uma forma concisa e poderosa de definir funções anônimas para tarefas específicas. Sua flexibilidade e facilidade de uso fazem deles uma ferramenta essencial para qualquer programador de Scheme. Entender como usar `lambda` efetivamente ajudará a escrever scripts mais limpos, modulares e eficientes.