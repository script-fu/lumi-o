...
```scheme
(+ 1 2)         ; Somma 1 e 2, risultato 3
(if #t 1 0)     ; Si valuta a 1 perché la condizione è vera
(list 1 2 3)    ; Crea un elenco: (1 2 3)
```
...
3. **Funzioni di ordine superiore**
   ...
   ```scheme
   (map (lambda (x) (* x x)) '(1 2 3 4))  ; Restituisce (1 4 9 16)
   ```
