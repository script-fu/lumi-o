...
Код Scheme состоит из **выражений**. Выражение принимает значение. Синтаксис единый: круглые скобки образуют вызов, в котором сначала указывается имя оператора или функции.

```scheme
(+ 1 2)         ; Складывает 1 и 2, результат 3
(if #t 1 0)     ; Возвращает 1, так как условие истинно
(list 1 2 3)    ; Создаёт список: (1 2 3)
```
...
3. **Функции высшего порядка**
   ...
   ```scheme
   (map (lambda (x) (* x x)) '(1 2 3 4))  ; Возвращает (1 4 9 16)
   ```
...
(add 3 5)  ; Возвращает 8
```

## Побочные эффекты и глобальное состояние
