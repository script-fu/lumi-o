...
- 每个测试均按其编写顺序进行评估。
- 当测试计算结果为 真（`#t`） 时，执行其相应的 **结果**，并且 `cond` 表达式停止评估进一步的测试。
- `else` 子句是可选的，如果没有一个测试评估为真，则用作后备。
...
2. **执行匹配结果**：
   - 当找到第一个评估为真 的测试 (`#t`) 时，将执行其**结果**。
   - 如果没有测试评估为真 并且存在 `else` 子句，则执行 **后备结果**。
...
- 第一个测试 `(< 3 2)` 的计算结果为 假（`#f`）。
- 第二个测试 `(= 3 3)` 的计算结果为 真（`#t`），因此返回 `"This will run"`。
- `else` 子句未执行，因为已找到匹配项。
