...
---
Na sua forma mais simples, a condicional `if` em Scheme avalia um teste e, com base no resultado, executa um dos dois blocos de código possíveis. A forma mais simples é assim:
...
Ambos os blocos de código `consequent` e `alternative` podem executar qualquer operação de Scheme válida, incluindo retornar valores, modificar variáveis ou executar procedimentos.

### Exemplos

#### Exemplo 1: Devolver um valor
...
- Como o teste é avaliado como verdadeiro (`#t`), o bloco **consequente** (`1`) é executado e o respetivo valor é retornado.

Resultado: **1**

#### Exemplo 2: Avaliando um bloco inicial

Nos casos em que precisa realizar várias ações quando a condição é verdadeira ou falsa, pode usar `begin` ou `let` para agrupá-las.
...
#### Exemplo 3: Avaliar uma instrução let
...
- A condicional `if` é uma ferramenta poderosa no Scheme para avaliar testes e executar blocos de código correspondentes.
