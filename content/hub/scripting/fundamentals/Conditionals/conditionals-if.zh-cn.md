...
- 如果 `test` 计算结果为 真（`#t`），则执行后续代码中的 **代码块**。该块可以返回一个值或执行其他操作，例如分配变量或打印输出。
...
- 由于测试结果为 真（`#t`），因此执行代码块`(lumi-message "True!")`，打印`"True!"`。
...
- 如果 `test` 计算结果为 真（`#t`），则执行 **后续** 代码块。
- 如果 `test` 的计算结果为 假（`#f`），则执行 **替代** 代码块。
...
2. **基于测试的结果**：
   - 如果 `test` 计算结果为 真（`#t`），则执行 **后续代码块**。
   - 如果 `test` 的计算结果为 假（`#f`），则执行 **替代代码块**。
...
- 由于测试评估结果为 真（`#t`），因此将执行 **后续** 块 (`1`) 并返回其值。

结果：**1**

#### 示例 2：评估开始块

如果您需要在条件为真 或 false 时执行多个操作，可以使用 `begin` 或 `let` 将它们组合在一起。
...
- 由于测试结果为 假（`#f`），因此执行 **alternative** 块：
...
- 由于测试结果为 真（`#t`），因此执行 **consequence** 块：
...
`if` 条件是Scheme 中用于评估测试和执行相应代码块的强大工具。
