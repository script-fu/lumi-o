...
```scheme
(define (sum-to-n n)
  (if (= n 0)                  ; Базовый случай: остановка, когда n равно 0
    0                          ; Базовый результат: сумма равна 0
    (+ n (sum-to-n (- n 1))))) ; Рекурсивный вызов: сложить текущее n с результатом меньшей подзадачи
```
