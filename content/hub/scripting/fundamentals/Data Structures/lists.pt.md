...
(car my-list) ; Recupera o primeiro elemento
(cdr my-list) ; Recupera o resto da lista
```
...
Chamando recursivamente `car` no `cdr` de uma lista, pode processar cada elemento um por um até que a lista seja percorrida. Isso forma a base de muitos algoritmos de processamento de lista.
...
```scheme
(list-ref (list 10 20 30 40) 2) ; Recupera o elemento no índice 2
```
...
As listas no Scheme podem conter outras listas como elementos, criando uma estrutura aninhada.
...
Para acessar elementos em uma lista aninhada, pode usar combinações de `car` e `cdr` para navegar pela estrutura.

#### Exemplo: Acessando Elementos

```scheme
(car nested-list) ; Recupera o primeiro elemento: (1 2)
(car (car nested-list)) ; Recupera o primeiro elemento da primeira sublista: 1
(cdr (car nested-list)) ; Recupera o resto da primeira sublista: (2)
(car (cdr (car nested-list))) ; Recupera o segundo elemento da primeira sublista: 2
```
...
```scheme
(car (cdr nested-list)) ; Recupera a segunda sublista: (3 4)
(car (car (cdr nested-list))) ; Recupera o primeiro elemento da segunda sublista: 3
```
