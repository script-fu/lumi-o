...
Embora vetores e listas possam parecer semelhantes, eles servem a propósitos diferentes na programação Scheme:

- As listas são mais comumente usadas para operações recursivas e estruturas dinâmicas, pois a implementação de nós ligados permite manipular eficientemente o início da lista e percorrê-la por decomposição recursiva.
...
(vector-ref my-vector 0) ; Recupera o elemento no índice 0
(vector-ref my-vector 1) ; Recupera o elemento no índice 1
```

#### Iteração: Processando cada elemento em um vetor

Pode iterar através de um vetor usando um loop ou recursão. O Scheme fornece `vector-length` para determinar o tamanho de um vetor. Aqui está um loop simples para imprimir todos os elementos de um vetor:
...
No Scheme, os vetores são mutáveis e passados por referência. Isso significa que quando passa um vetor para uma função, a função pode modificar diretamente o vetor original. Quaisquer alterações feitas no vetor dentro da função também serão refletidas fora da função. Esse comportamento é útil para partilhar e atualizar dados com eficiência em diversas funções, mas também requer cautela para evitar efeitos colaterais indesejados.
...
- **Efeitos colaterais:** Tenha cuidado ao partilhar vetores entre funções para evitar modificações não intencionais nos dados compartilhados.
...
Os vetores no Scheme podem conter outros vetores como elementos, criando uma estrutura aninhada.
...
```scheme
(vector-ref nested-vector 0) ; Recupera o primeiro elemento: #(1 2)
(vector-ref (vector-ref nested-vector 0) 1) ; Recupera o segundo elemento do primeiro vetor: 2
```
