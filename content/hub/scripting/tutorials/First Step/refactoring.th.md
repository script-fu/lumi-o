...
(define (send-message message output)
  (cond
    ;; ส่งไปยัง คอนโซลข้อผิดพลาด
    ((eq? output 'error-console)
       ;; ตั้งตัวจัดการเป็น คอนโซลข้อผิดพลาด
       (lumi-message-set-handler 2)
       (lumi-message message))
  ...
  ;; คืนค่าตัวจัดการข้อความเริ่มต้นไปยัง คอนโซลข้อผิดพลาด
  (lumi-message-set-handler 2))
...
ในตัวอย่างข้างต้น มีการใช้ชนิดข้อมูลที่เรียกว่าสัญลักษณ์ เช่น 'gui' สัญลักษณ์จะถูกส่งผ่านเป็นพารามิเตอร์ไปยังฟังก์ชันส่งข้อความ และสามารถใช้เพื่อตัดสินใจตามเงื่อนไขง่ายๆ เช่นเดียวกับคีย์สัญลักษณ์ พวกมันเป็นตัวระบุที่ไม่ซ้ำกัน สำหรับข้อมูลเพิ่มเติมเกี่ยวกับสัญลักษณ์ โปรดไปที่ [หน้านี้](/hub/scripting/fundamentals/variables-and-scope/symbols/)

### ลดความซับซ้อนของฟังก์ชันหลัก

ในฟังก์ชันดั้งเดิม (scheme-hello-world) ตรรกะทั้งหมดสำหรับการส่งข้อความไปยังเอาต์พุตต่างๆ (GUI, คอนโซลข้อผิดพลาด, Terminal) ได้ถูกผสมเข้ากับฟังก์ชันหลัก หลังจากการปรับโครงสร้างใหม่ ฟังก์ชันหลักจะเน้นไปที่ **สิ่งที่ต้องทำ** เพื่อส่งข้อความไปยังปลายทางต่างๆ
...
ในทางตรงกันข้าม ฟังก์ชัน **(ส่งข้อความ)** คือตำแหน่งที่มีตรรกะโดยละเอียดอยู่ ตอนนี้จัดการรูปแบบการทำงานที่แตกต่างกันสำหรับแต่ละเอาต์พุต (GUI, คอนโซลข้อผิดพลาด, Terminal) ฟังก์ชันนี้ซับซ้อนกว่าเดิมเล็กน้อย แต่ตอนนี้ **รวมศูนย์** และ **แยก** แล้ว
