...
Теперь, когда проверка вынесена в отдельную функцию, функция `send-message` может сосредоточиться только на отправке сообщения. Это будет намного проще, поскольку он решает только конкретную задачу по перенаправлению сообщения в правильный пункт назначения.
