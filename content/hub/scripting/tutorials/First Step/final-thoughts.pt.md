...
---
Já tem um plug-in de procedimento funcional e uma pequena biblioteca auxiliar. Esta série apresentou os padrões básicos que usará na maioria dos scripts Lumi:

- Funções: Os blocos de construção dos plug-ins.
- Refatoração: Mais estrutura do código mantendo a funcionalidade.
- Bibliotecas de Código: Centralizando funções reutilizáveis para manter o código limpo e modular.
- Técnicas de validação: Garantir que as entradas sejam válidas antes de executar a lógica principal central.

Também viu os fundamentos do uso do Git para acompanhar alterações e manter uma estrutura de projeto limpa. Esse fluxo de trabalho facilita a iteração sem perder versões de trabalho.

Aqui está a versão final do código de plug-in principal:
...
- Mova auxiliares reutilizáveis para um ficheiro de biblioteca dedicado.
- Mantenha os plug-ins pequenos e nomeie os procedimentos de acordo com o que eles fazem.
- Adicione validação nos limites (entradas, caminhos de ficheiros, opções de menu).

Mantenha o resultado final como dois ficheiros no repositório de plug-ins:
