...
- **enviar para ficheiro**: Salva mensagens em um ficheiro de log.
- **send-to-logger**: Integração com um sistema de registo externo.
...
- **Consistência**: usar as mesmas funções de validação e tratamento de mensagens garante um comportamento consistente em toda a aplicação.

A **Biblioteca de Mensagens** é o início de uma estrutura mais ampla que pode simplificar a forma como as mensagens são gerenciadas no projeto. À medida que a biblioteca cresce, novos plug-ins podem acessá-la facilmente para enviar mensagens para onde quer que precisem.

Podemos ajustar a estrutura do ficheiro:
