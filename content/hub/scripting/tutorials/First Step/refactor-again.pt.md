...
(define (send-message message output)
 ...
 (cond
 ;; Enviar para a consola de mensagens
 ...
 ;; Restaurar o manipulador de mensagens padrão para a consola de mensagens
 (lumi-message-set-handler 2))
```

### Dividindo ainda mais: separe cada manipulador de saída

Cada tipo de saída de mensagem (GUI, console de mensagens, Terminal) pode ser movido para a própria função. Isso permite testes, modificações e extensões potenciais mais fáceis no futuro.
...
Como a validação é uma parte importante para garantir que a mensagem e a saída estejam corretas, faz sentido que cada função `send-*` execute a própria validação. Isso garante que não importa qual saída seja chamada, sempre verificaremos as entradas primeiro.
...
Ao mover a validação para cada função send-to-*, nós as tornamos reutilizáveis como funções independentes. Isso significa que podemos chamar qualquer uma das funções send-to-gui, send-to-error-console ou send-to-terminal diretamente, sem depender da função send-message dispatcher. Cada uma dessas funções agora lida totalmente com a própria lógica e pode ser usada de forma independente em outras partes do código ou em outros plug-ins, tornando o código mais modular e flexível.
...
- **Extensibilidade**: Adicionar novos tipos de saída é simples. simplesmente define uma nova função como `send-to-file` ou `send-to-logger` e, em seguida, adiciona um caso na instrução `cond`.
- **Reutilização**: cada uma dessas funções de manipulação de saída pode ser reutilizada em outro lugar do projeto ou compartilhada entre vários plug-ins.
- **Consistência**: Ao reutilizar a função de validação em cada função `send-to-*`, garante-se que todas as saídas sejam devidamente validadas, tornando o código mais robusto.
...
;; Propósito: Envia uma mensagem para a consola de mensagens
(define (send-to-error-console message)
 ;; Validar a mensagem antes de prosseguir
 (validate-message message 'error-console)
 (lumi-message-set-handler 2)
