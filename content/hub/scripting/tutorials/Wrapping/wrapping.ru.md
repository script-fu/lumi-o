...
Lumi поставляет набор готовых оболочек, автоматически загружаемых при запуске, поэтому они доступны в любом плагине или в консоли схемы без какого-либо вызова `(load ...)`. Эти библиотеки — `common.scm`, `files.scm`, `gegl.scm`, `images.scm`, `layers.scm`, `parasites.scm` и `paths.scm` — построены точно по тому же принципу, что и примеры выше: они дают ясность имена для операций низкого уровня, скрыть повторяющийся шаблон и предоставить единое место для обновления в случае изменения базовой команды.Например, `images.scm` предоставляет `image-get-open-list` в качестве читаемой оболочки вокруг необработанного вызова PDB, а `files.scm` предоставляет помощники построения пути, которые в противном случае потребовали бы повторения цепочек `string-append`.

Вы можете просмотреть каждое экспортированное имя, прочитать его документацию и посмотреть, из какой библиотеки оно взято, в **[Utility Browser](@@LUMI_TOKEN_21@@)** (Справка → Программирование → Браузер служебных программ). Это практическая демонстрация переноса в масштабе и полезный источник шаблонов, которые можно позаимствовать при создании собственной вспомогательной библиотеки.
