...
**[Utility Browser]({{< ref "/hub/scripting/reference/utility-browser" >}})** (ヘルプ → プログラミング → ユーティリティ ブラウザ) で、エクスポートされたすべての名前を参照し、その docstring を読み、どのライブラリからのものかを確認できます。これは大規模なラッピングの実践的なデモンストレーションであり、独自のヘルパー ライブラリを構築するときに借用できるパターンの有用なソースです。
