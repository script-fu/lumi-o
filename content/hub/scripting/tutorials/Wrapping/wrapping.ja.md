...
### 出荷されたラッパー: ユーティリティ標準ライブラリ

Lumi には、起動時に自動的にロードされる既製のラッパーのセットが同梱されているため、`(load ...)` を呼び出すことなく、任意のプラグインまたは Scheme コンソールで使用できます。これらのライブラリ (`common.scm`、`files.scm`、`gegl.scm`、`images.scm`、`layers.scm`、`parasites.scm`、および `paths.scm`) は、上記の例とまったく同じ原理に基づいて構築されています。つまり、低レベルの操作に明確な名前が付けられています。反復的なボイラープレートを非表示にし、基礎となるコマンドが変更された場合に更新できる単一の場所を提供します。たとえば、`images.scm` は生の PDB 呼び出しの読み取り可能なラッパーとして `image-get-open-list` を提供し、`files.scm` は `string-append` チェーンの繰り返しが必要となるパス構築ヘルパーを公開します。

**[ユーティリティ ブラウザ]({{< ref "/hub/scripting/reference/utility-browser" >}})** (ヘルプ → プログラミング → ユーティリティ ブラウザ) で、エクスポートされたすべての名前を参照し、その docstring を読み、どのライブラリからのものかを確認できます。これは大規模なラッピングの実践的なデモンストレーションであり、独自のヘルパー ライブラリを構築するときに借用できるパターンの有用なソースです。

### 結論

関数のラッピングは、Scheme 開発を簡素化し、スクリプトをより読みやすく、保守しやすく、堅牢にする強力な方法です。複雑さをカプセル化し、必要な詳細のみを公開することで、プラグインを作成するためのより構造化されたアプローチを作成します。

このアプローチから得られる重要なポイントは次のとおりです。

- **繰り返しのタスクを簡素化** – 低レベルのコマンドを手動で繰り返す代わりに、再利用可能な関数を作成します。
