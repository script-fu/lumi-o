...
Lumi 附帶了一組在啟動時自動載入的現成包裝器，因此它們可以在任何插件或方案控制台中使用，無需任何 `(load ...)` 呼叫。這些函式庫 — `common.scm`、`files.scm`、`gegl.scm`、`images.scm`、`layers.scm`、@@LUMI_TOKEN_15@MTOKEN_15@STO@LUMI_15@4_LUMI_15@的建造原理與上述範例完全相同：它們給出了明確的資訊為低階操作命名，隱藏重複的樣板文件，並在底層命令發生變化時提供一個更新位置。例如，`images.scm` 提供 `image-get-open-list` 作為原始 PDB 呼叫的可讀包裝器，`files.scm` 公開路徑構建幫助程序，否則需要重複的 `string-append` 鏈。
