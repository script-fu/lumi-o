...
检索 ***x*** 坐标。然而，雖然功能齊全，但表現力並不強。相反，我們可以用更合適的定義來包裝***first-item***，以使我們的意圖更清晰。
...
Lumi 附帶了一組在啟動時自動載入的現成包裝器，因此它們可以在任何插件或方案控制台中使用，無需任何 `(load ...)` 呼叫。這些函式庫（`common.scm`、`files.scm`、`gegl.scm`、`images.scm`、`layers.scm`、`parasites.scm` 和`paths.scm`）的建構原理與上述範例完全相同：它們給出了明確的資訊為低階操作命名，隱藏重複的樣板文件，並在底層命令發生變化時提供一個更新位置。例如，`images.scm` 提供 `image-get-open-list` 作為原始 PDB 呼叫的可讀包裝器，`files.scm` 公開路徑構建幫助程序，否則需要重複的 `string-append` 鏈。

您可以瀏覽每個匯出的名稱，閱讀其文件字串，並在 **[Utility Browser](@@LUMI_TOKEN_21@@)**（幫助 → 程式設計 → 實用程式瀏覽器）中查看它來自哪個庫。它是大規模包裝的實際演示，也是建立您自己的幫助程式庫時可以藉用的有用模式來源。
...
- **簡化重複性任務** – 我們建立可重複使用的函數，而不是手動重複低階命令。
