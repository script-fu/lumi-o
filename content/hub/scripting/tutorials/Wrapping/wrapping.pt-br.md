...
Você pode navegar por cada nome exportado, ler sua documentação e ver de qual biblioteca ele vem em **[Utility Browser]({{< ref "/hub/scripting/reference/utility-browser" >}})** (Ajuda → Programação → Navegador de Utilitários). É uma demonstração prática de empacotamento em escala e uma fonte útil de padrões para emprestar ao construir sua própria biblioteca auxiliar.
