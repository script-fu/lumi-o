...
---
Trabalhar com ficheiros e diretórios é essencial para o desenvolvimento de Scheme. Esteja a guardar resultados, carregando recursos ou organizando a estrutura do projeto, compreender as operações dos ficheiros tornará os scripts mais robustos e fáceis de usar.

Esta página aborda tarefas comuns de ficheiros e diretórios: leitura de caminhos, criação de diretórios e coleta de entradas de pastas por meio de parâmetros GUI.

## Diretório inicial do utilizador

Lumi é somente Linux, então o diretório inicial do utilizador vem da variável de ambiente `HOME`.

Para obter o diretório inicial do utilizador como uma string:
...
Podemos pedir ao utilizador um local de diretório na caixa de diálogScheme para um plug-in.
...
Se estiver interessado nos detalhes da implementação, pesquise `validate-path-and-dir` na fonte do plug-in.
...
Nota: Esta função também usa ```file-exists?``` integrado para pular chamadas desnecessárias. Retorna #t se o ficheiro ou diretório indicado existir e #f se não existir ou se não estiver acessível ao utilizador solicitante.
