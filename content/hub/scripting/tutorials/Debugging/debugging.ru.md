...
2. `serialize-item` — преобразует различные типы данных Scheme в строковое представление.
...
Функция `warning-message` работает аналогично `debug-message`, но отображает предупреждения, даже если отладка отключена.
