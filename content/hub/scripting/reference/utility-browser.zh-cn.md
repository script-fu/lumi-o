...
- **搜索框** — 在您键入时按名称进行过滤（不区分大小写的子字符串匹配）。
- **种类过滤器** — 将结果范围缩小到 `procedure`、`variable` 或 `syntax`。
...
实用程序浏览器与**过滤器→ Script-Fu → 控制台→ 浏览**（过程浏览器）是分开的。过程浏览器列出了 PDB 注册的过程。实用程序浏览器列出了有意位于 PDB *外部的帮助器定义 - 它们仅限于方案并且没有 C 绑定。