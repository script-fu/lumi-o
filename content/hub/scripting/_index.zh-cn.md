---
title: "脚本"
type: docs
translation_provenance: ai-reviewed
translation_lock: true
translation_source_sha256: 5faeaef488b8a330727b633f7808b8e1982e8633d9894b959c2a4080fff7aeca
url: "hub/scripting/_index"
---
Scheme 脚本可用于自动化重复性工作、构建自定义工具，并用您自己的插件扩展 Lumi。

通过这些指南学习语言基础、按步骤完成教程、建立良好的工作流程，并探索可读可改的示例插件。

{{< cards >}}
  {{< card link="fundamentals/" title="基础" icon="variable" >}}
  {{< card link="tutorials/" title="教程" icon="sparkles" >}}
  {{< card link="tools/" title="工具" icon="cog" >}}
  {{< card link="plug-ins/" title="插件" icon="collection" >}}
  {{< card link="reference/" title="参考" icon="book-open" subtitle="内置实用程序与应用内浏览工具" >}}
{{< /cards >}}
