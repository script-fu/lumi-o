---
title: "Инструменты"
type: docs
translation_provenance: ai-reviewed
translation_lock: true
translation_source_sha256: c4c14f8ed284abd28bdb6e567187a850ba5eb7815fdb376bab03cc8c62091640
---
Эти руководства описывают минимальный набор инструментов для написания Scheme-плагинов для Lumi в Linux. Используйте систему контроля версий с самого начала. Git даёт надёжную историю, а размещение на GitLab или GitHub упрощает синхронизацию и ревью изменений.

{{< cards >}}
  {{< card link="git" title="Git" icon="folder-tree" subtitle="Контроль версий для разработки на Scheme" >}}
  {{< card link="visual-studio-code" title="Visual Studio Code" icon="code" subtitle="Настройка редактора и расширения для Scheme" >}}
  {{< card link="ai-assisted" title="С поддержкой ИИ" icon="sparkles" subtitle="Писать и рефакторить плагины с помощью ИИ-агентов" >}}
{{< /cards >}}
