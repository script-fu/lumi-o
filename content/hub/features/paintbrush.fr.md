...
### Pinceaux raster
Images de pinceau bitmap chargées à partir de fichiers `.png` ou `.vbr`. Prend en charge la transparence alpha et les cadres de canal animés.
...
### Têtes de brosse
Peignez avec plusieurs têtes de pinceau indépendantes disposées en anneau de formation autour du tracé. Les commandes apparaissent dans l'extenseur **Têtes de brosse** du panneau d'options de l'outil.- **Têtes** : Nombre de têtes de brosse simultanées (1 à 16).
...
- **Suit** (0,0–1,0) : la force avec laquelle l'anneau de formation suit la direction de déplacement de la course. À 1,0 (par défaut), l'anneau est toujours perpendiculaire à la direction de déplacement. À 0,0, il est verrouillé sur la valeur statique **Angle**. Des valeurs intermédiaires se mélangent entre les deux orientations. Ceci est indépendant du système Dynamics — aucune configuration de dynamique d’angle n’est requise.
- **Angle** (0–360°) : orientation statique de l'anneau de formation, utilisé lorsque **Follows** est inférieur à 1,0. Lorsque **Verrouiller sur la vue** est actif, l'angle est automatiquement compensé pour la rotation du canevas.
- **Variation** : variation de la taille par tête et biais de pression appliqués à la dynamique.
- **Variation d'opacité** : variation d'opacité par tête, indépendante de la variation de taille.
- **Seed** : Correction de la graine aléatoire pour la variation par tête. S'applique uniquement lorsque **Random Bristles** est désactivé.
- **Poils aléatoires** : randomisez le caractère des poils à chaque coup (ignore la graine).
- **Images indépendantes** : pour les pinceaux animés : lorsque cette option est activée, chaque tête avance indépendamment son image d'animation.

### Options supplémentaires

Dans l'expandeur **Options supplémentaires** (réduit par défaut) :
...
- **Réinitialiser l'animation** : pour les pinceaux animés : lorsque cette option est activée, l'animation redémarre à partir de l'image 0 à chaque nouveau trait ; lorsqu'elle est désactivée, elle continue à partir de l'endroit où le trait précédent s'est terminé.