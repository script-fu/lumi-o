...
type: docs
---
Lumi는 단일 대화 상자에서 액세스할 수 있는 두 개의 독립적인 복구 시스템(자동 백그라운드 저장 및 수동 증분 체크포인트)을 유지 관리합니다.
...
새로운 `primary-NN.lum/` 기준선은 **파일 → 저장** 후에 작성됩니다. 이어서 **파일 → 증분 저장**을 누르면(`Ctrl+I`) 마지막 기준선 이후 변경된 버퍼만 포함하는 `delta-NNNN.lum/` 하위 디렉터리가 생성됩니다. 자동 저장 델타와 수동 저장 델타는 별도의 카운터를 사용하므로 서로의 기록을 방해하지 않습니다.

저장된 `.lum` 파일에 대해 저장 증분을 **항상 사용 가능**합니다.

1. **파일** → **저장**(`Ctrl+S`)을 사용하여 메인 프로젝트 파일을 생성하거나 업데이트합니다.
2. **파일** → **증분 저장**(`Ctrl+I`)을 사용하여 복구 체크포인트를 생성합니다.
3. 또 다른 전체 **파일** → **저장** 후, 다음 `Ctrl+I`은 새 델타를 생성하기 전에 새로운 `primary-NN.lum/` 기준선을 작성합니다.

`RECOVERED_` 접두사로 명명된 복구된 파일은 저장 증분을 사용할 수 있게 되기 전에 먼저 정상적으로 저장되어야 합니다.

기본 저장보다 최신 증분 저장이 있는 `.lum` 파일을 열면 Lumi는 가장 최근 체크포인트를 로드하라는 **증분 저장 감지** 프롬프트를 표시합니다.
...
시간이 지남에 따라 복구 상태가 누적되면 상당한 디스크 공간이 소모될 수 있습니다. **이전 상태 정리...** 버튼(대화 상자 왼쪽 하단)은 활성 탭(자동 저장 또는 증분)에 대한 정리 프롬프트를 엽니다.
...
**최근 항목 유지**를 `0`으로 설정하면 모든 복구 상태가 삭제됩니다. 전체 정리 후 다음 `Ctrl+I`에서는 새로운 기본 저장을 작성합니다.
