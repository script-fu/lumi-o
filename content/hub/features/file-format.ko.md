...
증분 저장은 **파일** → **증분 저장**(`Ctrl+I`)을 통해 가능합니다. 일반적인 **파일** → **저장**(`Ctrl+S`)을 바꾸지 않고 프로젝트 내부에 수동 복구 체크포인트를 생성합니다. 전체 저장은 여전히 ​​기본 `.lum` 프로젝트를 업데이트하는 반면, 저장 증분은 빠른 체크포인트에 필요한 수정된 레이어만 작성합니다.
...
Lumi는 정기적으로 **별도의 캐시 위치**(`~/.cache/lumi/autosave/`)에 변경 사항을 자동으로 저장합니다. 자동 저장은 작업 파일과 독립적이며 수정하지 않습니다. 간격과 캐시 위치는 **편집 → 기본 설정 → 성능**에서 구성할 수 있습니다.
...
- **파일** → **증분 저장**(Ctrl+I): 현재 `.lum` 파일에 대한 증분 복구 체크포인트를 생성합니다.
- **파일** → **다른 이름으로 저장**(Shift+Ctrl+S): 새 `.lum` 파일에 저장합니다. 다른 이름으로 저장 대화 상자에는 새 프로젝트 파일에 대한 압축 옵션이 포함되어 있습니다.
...
## 프로젝트 복구Lumi는 **파일** → **이미지 복구**에서 액세스할 수 있는 자동 백그라운드 저장 및 수동 증분 체크포인트를 유지합니다. 자세한 내용은 [파일 복구](../recovery) 페이지를 참조하세요.
...
레이어 버퍼의 이름은 순차적으로 번호가 지정되지 않고 레이어(`layer-Background.geglbuf`)를 따라 지정됩니다. 레이어 이름의 공백은 밑줄로 저장됩니다. 그룹 레이어에는 `-GROUP` 접미사가 붙습니다. 마스크는 레이어 이름(`mask-Background.geglbuf`)을 공유합니다.

각 `recovery/primary-NN.lum/`은 전체 기준 저장입니다. 후속 `Ctrl+I`을 누르면 마지막 기준 이후 수정된 버퍼만 포함된 `delta-NNNN.lum/` 하위 디렉터리가 추가되며, 체크포인트를 유지하면 프로젝트 크기에 관계없이 빠르게 저장됩니다.
