...
- **방향 따르기**(0.0–1.0): 형성 링이 스트로크 이동 방향을 얼마나 강하게 추적하는지입니다. 1.0에서는 링이 항상 이동 방향에 수직입니다. 0.0에서는 정적 **Angle** 값으로 고정됩니다.
- **압력 변화**: 역학 곡선을 통해 독립적인 압력 바이어스로 적용되는 헤드당 크기 변화입니다.
- **불투명도 변형**: 크기 변형과 무관한 헤드당 불투명도 변형입니다.
- **문자 시드** (0–255): 머리당 문자에 대한 고정 시드(크기, 채우기 간격 위치). 동일한 씨앗은 스트로크마다 동일한 형태를 재현합니다. **머리 캐릭터 무작위화**가 켜져 있으면 감도가 둔화됩니다.
...
- **분산 각도**(0–90°, 기본값 10°): 각 헤드를 스트로크 방향에서 바깥쪽으로 새로 무작위 각도로 이 값까지 부채꼴로 흩뿌립니다. 90°로 고정되어 머리가 뒤로 향하지 않습니다.
- **앞으로 분산형**(0–4000px): 스트로크 방향 앞에서 최대 무작위 분산형입니다. 매 Dab마다 독립적으로 다시 굴립니다.
- **뒤로 분산형**(0~4000px): 획 뒤의 최대 무작위 분산형입니다. 머리는 여전히 앞을 향하고 있습니다. 변위 방향만 반전됩니다. Forward와 Backward는 동시에 0이 아닐 수 있습니다.
- **Scatter Size Balance** (0.0–1.0): 큰 머리에 대한 최소 분산 가중치입니다. 0에서는 큰 머리가 스트로크에 가깝게 착지합니다. 1에서는 모든 머리가 크기에 관계없이 균등하게 분산됩니다.
- **분산 크기 임계값**(1~100픽셀): 이 픽셀 반경보다 작은 머리는 전체 거리에서 분산됩니다. 더 큰 헤드는 스트로크에 점점 더 가깝게 당겨집니다.

#### 무작위화

- **머리 문자 무작위화**: 매 스탬프마다 머리별 문자 값(크기, 분산 위치)을 다시 그려서 획을 따라 형성이 완전히 혼란스러워집니다. **문자 시드**를 재정의합니다.
- **애니메이션 프레임 무작위화**: 애니메이션 브러시의 경우: 각 헤드는 애니메이션 프레임을 독립적으로 진행합니다.

### 추가 옵션**추가 옵션** 확장기(기본적으로 접혀 있음):
