...
- Выбор слоя не активируется во время операций с инструментом **Трансформировать**; Alt здесь имеет другое значение.
...
- **Цвет блокировки**: фиксирует рисование по определенному цвету: текущему цвету переднего плана на момент применения блокировки. Последующие штрихи на этом слое используют сохраненный цвет независимо от активного цвета переднего плана. Разблокировка удаляет сохраненный цвет.
...
Во всплывающем окне отображается стек фильтров для слоя, где каждый зафиксированный эффект указан по имени с переключателем видимости рядом с ним.
...
- **Композитный режим**: управляет взаимодействием альфа-слоя с фоном. Доступные варианты: «Объединение» (влияет на все области, значение по умолчанию для обычного режима), «Обрезать до фона» (влияет только на области с существующим содержимым, значение по умолчанию для большинства других режимов наложения) и «Пересечение».
...
**Смещение слоя**: счетчики X и Y управляют положением слоя на холсте. Изменения применяются немедленно, а не при закрытии диалогового окна.

Если слой имеет маску, ниже отображаются **Смещения маски** (вертушки X и Y для независимого положения маски).
