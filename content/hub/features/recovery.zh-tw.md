...
Lumi 在编辑时定期保存您作品的背景快照。自動儲存被寫入 **單獨的快取目錄**，使工作 `.lum` 檔案保持不變：
...
在**檔案→儲存**之後寫入新的`primary-NN.lum/`基線。隨後的**檔案 → 儲存增量**按 (`Ctrl+I`) 建立 `delta-NNNN.lum/` 子目錄，僅包含自上次基線以來變更的緩衝區。自動儲存增量和手動儲存增量使用單獨的計數器，因此它們不會幹擾彼此的歷史記錄。

對於已儲存的 `.lum` 文件，儲存增量**始終可用**：

1. 使用 **檔案** → **儲存** (`Ctrl+S`) 建立或更新主項目檔案。
2. 使用 **檔案** → **儲存增量** (`Ctrl+I`) 建立復原檢查點。
3. 在另一個完整的 **檔案** → **儲存** 之後，下一個 `Ctrl+I` 在建立新的增量之前寫入新的 `primary-NN.lum/` 基準。

以 `RECOVERED_` 前綴命名的恢復檔案必須先正常儲存，然後才能使用「儲存增量」。

當您開啟具有比主儲存更新的增量儲存的 `.lum` 檔案時，Lumi 會顯示 **偵測到增量儲存** 提示，提示載入最新的檢查點。
...
隨著時間的推移累積復原狀態可能會消耗大量磁碟空間。 **清理舊狀態...**按鈕（對話方塊左下角）開啟活動標籤（自動儲存或增量）的清理提示。
...
將 **保留最新** 設定為 `0` 會刪除所有恢復狀態。完全清理後的下一個 `Ctrl+I` 將寫入新的主保存。
