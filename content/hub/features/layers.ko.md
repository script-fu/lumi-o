...
- **레이어 마스크**: 회색조 마스크를 적용하여 레이어 가시성을 픽셀별로 제어합니다. 마스크에 흰색을 칠하면 드러납니다. 검은 색은 은폐합니다. 회색은 부분적인 불투명도를 제공합니다.
...
**수정** 및 **제거**는 레이어에서 픽셀 잠금이 활성화된 경우 차단됩니다. Filters cannot be reordered while one is actively being edited.
...
대화 상자의 아래쪽 섹션에는 영구 기생충에 대한 스크롤 가능한 이름/값 테이블이 포함되어 있습니다(레이어에 연결된 임의의 키-값 메타데이터). 이러한 값은 프로젝트와 함께 저장되며 Scheme 스크립팅 인터페이스에서 액세스할 수 있습니다.
