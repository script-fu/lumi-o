...
{{< cards >}}
  ...
  {{< card link="file-format" title="文件格式 (.lum)" icon="document-text" subtitle="儲存圖層、元資料、ICC 設定檔和復原資料的開放容器格式。" >}}
  {{< card link="recovery" title="文件恢復" icon="refresh" subtitle="自動增量復原快照可防止工作崩潰和斷電。" >}}
  {{< card link="layers" title="圖層和無損編輯" icon="collection" subtitle="基於圖層的合成，具有混合模式、蒙版、群組和非破壞性調整圖層。" >}}
  {{< card link="paintbrush" title="畫筆工具" icon="pencil-alt" subtitle="具有穩定、平滑和多頭刷結構的壓力、速度、傾斜和間距動態。" >}}
  {{< card link="palette-editor" title="調色板編輯器" icon="color-swatch" subtitle="創建和管理命名的顏料調色板，其中包含涵蓋全光譜和有限調色板的捆綁套件。" >}}
  {{< card link="palette-map" title="調色盤圖" icon="view-grid" subtitle="將調色板顏料映射到軸的交互式 2D 網格，用於直接拾色和探索。" >}}
