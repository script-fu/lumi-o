...
### Pędzle rastrowe
Obrazy pędzli bitmapowych załadowane z plików `.png` lub `.vbr`. Obsługuje przezroczystość alfa i animowane ramki potoków.
...
### Główki szczoteczek
Maluj wieloma niezależnymi główkami pędzli ułożonymi w pierścień wokół ścieżki pociągnięcia. Elementy sterujące pojawiają się w ekspanderze **Główki pędzli** w panelu opcji narzędzia.- **Głowice**: Liczba jednoczesnych główek szczoteczki (1–16).
- **Rozmiar**: Renderowany rozmiar każdej główki w stosunku do globalnego rozmiaru pędzla (0,1–1,0).
- **Sztywność**: Jak sztywno promień orbity dopasowuje się do rozmiaru pędzla skalowanego dynamiką. 0 = orbita śledzi rozmiar dynamiki; 1 = orbita pozostaje niezmienna w rozmiarze podstawowym.
- **Następuje** (0,0–1,0): Jak mocno pierścień formujący śledzi kierunek ruchu skoku. Przy 1,0 (domyślnie) pierścień jest zawsze prostopadły do ​​kierunku jazdy. Przy wartości 0,0 jest ona zablokowana na statyczną wartość **Kąta**. Wartości pośrednie łączą obie orientacje. Jest to niezależne od systemu Dynamics — nie jest wymagana żadna konfiguracja dynamiki kąta.
- **Kąt** (0–360°): Statyczna orientacja pierścienia formującego, stosowana, gdy **Podąża** jest poniżej 1,0. Kiedy **Zablokuj widok** jest aktywny, kąt jest automatycznie kompensowany przy obrocie płótna.
- **Odmiana**: Zmienność wielkości na głowę i odchylenie ciśnienia zastosowane do dynamiki.
- **Zmienność krycia**: Zmienność krycia na głowicę, niezależna od zmiany rozmiaru.
- **Nasiona**: Naprawiono losowe nasiona dla zmienności na głowę. Ma zastosowanie tylko wtedy, gdy opcja **Losowe włosie** jest wyłączona.
- **Losowe włosie**: Losowy charakter włosia przy każdym pociągnięciu (ignoruje nasiona).
- **Niezależne klatki**: W przypadku pędzli animowanych — gdy ta opcja jest włączona, każda głowica niezależnie przesuwa klatkę animacji.

### Dodatkowe opcje

W ekspanderze **Opcje dodatkowe** (domyślnie zwiniętym):
